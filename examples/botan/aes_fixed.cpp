...
inline const std::vector<uint32_t>& AES_TE() __attribute__((always_inline))
{
   //auto compute_TE = []() -> std::vector<uint32_t> {
      ...
      static const std::vector<uint32_t> E = TE;
      return E;
   // };
                   ...
                   const uint8_t in[], uint8_t out[])
{
   ...
   for(size_t i = 0; i < 1; ++i)
   {
      ...
      T0=*((uint32_t *)(in + 16*i));
      T1=*((uint32_t *)(in + 16*i+4));
      T2=*((uint32_t *)(in + 16*i+8));
      T3=*((uint32_t *)(in + 16*i+12));
...
extern "C" {
void aes_fixed_wrapper(uint8_t* key, uint8_t* in_buf) {
   std::vector<uint32_t> m_EK, m_DK;
   std::vector<uint8_t> m_ME, m_MD;
...
CT_FUZZ_SPEC(void, aes_fixed_wrapper, uint8_t* key, uint8_t* in_buf) {
  __ct_fuzz_ptr_len(key, 24, 24);
  __ct_fuzz_ptr_len(in_buf, 64, 64);
}

CT_FUZZ_SEED(void, aes_fixed_wrapper, uint8_t*, uint8_t*) {
  ...
  PRODUCE(aes_fixed_wrapper, const_cast<uint8_t*>(key), const_cast<uint8_t*>(in_buf))
}
}
