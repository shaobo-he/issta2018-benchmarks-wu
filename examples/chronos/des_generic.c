...
inline void dkey(uint32_t *pe, const uint8_t *k) __attribute__((always_inline))
{
	/* K&R: long is at least 32 bits */
...
inline int des_setkey(const uint8_t *key, struct des_ctx *dctx, unsigned int keylen) __attribute__((always_inline))
{
	uint32_t tmp[DES_EXPKEY_WORDS];
	int ret;
