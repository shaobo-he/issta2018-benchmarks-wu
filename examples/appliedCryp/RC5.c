...
void rc5_encrypt(rc5_ctx *, uint32_t *, int);
void rc5_decrypt(rc5_ctx *, uint32_t *, int);
...
void rc5_init(rc5_ctx *c, int rounds){
	c->nr = rounds;
	c->xk = (uint32_t *) malloc(4*(rounds*2+2));
}
...
void rc5_decrypt(rc5_ctx *c, uint32_t *data, int blocks){
	uint32_t *d,*sk;
	int h,i,rc;
	d = data;
	sk = (c->xk)+2;
...
void main(void){
	...
	for(i=0;i<8;i+=2) printf("Block %01d = %08x %08x\n", i/2,data[i],data[i+1]);
